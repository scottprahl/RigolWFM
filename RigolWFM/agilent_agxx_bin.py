...
class AgilentAgxxBin(KaitaiStruct):
    ...
    def __init__(self, _io, _parent=None, _root=None):
        super(AgilentAgxxBin, self).__init__(_io)
        self._parent = _parent
        self._root = _root or self
        self._read()

    def _read(self):
        self.file_header = AgilentAgxxBin.FileHeader(self._io, self, self._root)
        self.waveforms = []
        for i in range(self.file_header.n_waveforms):
            self.waveforms.append(AgilentAgxxBin.Waveform(self._io, self, self._root))
    ...
    class DataHeader(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(AgilentAgxxBin.DataHeader, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
        ...
        def _read(self):
            ...
            self.buffer_type = KaitaiStream.resolve_enum(AgilentAgxxBin.BufferTypeEnum, self._io.read_u2le())
    ...
    class FileHeader(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(AgilentAgxxBin.FileHeader, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class Waveform(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(AgilentAgxxBin.Waveform, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()

        def _read(self):
            self.wfm_header = AgilentAgxxBin.WaveformHeader(self._io, self, self._root)
            self.buffers = []
            for i in range(self.wfm_header.n_buffers):
                self.buffers.append(AgilentAgxxBin.WaveformBuffer(self._io, self, self._root))
    ...
    class WaveformBuffer(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(AgilentAgxxBin.WaveformBuffer, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()

        def _read(self):
            self.data_header = AgilentAgxxBin.DataHeader(self._io, self, self._root)
    ...
    class WaveformHeader(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(AgilentAgxxBin.WaveformHeader, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
        ...
        def _read(self):
            self.header_size = self._io.read_u4le()
            self.waveform_type = KaitaiStream.resolve_enum(AgilentAgxxBin.WaveformTypeEnum, self._io.read_u4le())
            ...
            self.x_units = KaitaiStream.resolve_enum(AgilentAgxxBin.UnitEnum, self._io.read_u4le())
            self.y_units = KaitaiStream.resolve_enum(AgilentAgxxBin.UnitEnum, self._io.read_u4le())
