...
class Lecroy23Be(KaitaiStruct):
    ...
    def __init__(self, _io, _parent=None, _root=None):
        super(Lecroy23Be, self).__init__(_io)
        self._parent = _parent
        self._root = _root or self
        self._read()

    def _read(self):
        self.wavedesc = Lecroy23Be.Wavedesc(self._io, self, self._root)
    ...
    class TriggerTimestamp(KaitaiStruct):
        ...
        """
        def __init__(self, _io, _parent=None, _root=None):
            super(Lecroy23Be.TriggerTimestamp, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class TrigtimeEntry(KaitaiStruct):
        ...
        """
        def __init__(self, _io, _parent=None, _root=None):
            super(Lecroy23Be.TrigtimeEntry, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class Wavedesc(KaitaiStruct):
        ...
        """
        def __init__(self, _io, _parent=None, _root=None):
            super(Lecroy23Be.Wavedesc, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
        ...
        def _read(self):
            ...
            self.comm_type = KaitaiStream.resolve_enum(Lecroy23Be.CommTypeEnum, self._io.read_u2be())
            self.comm_order = KaitaiStream.resolve_enum(Lecroy23Be.CommOrderEnum, self._io.read_u2be())
            ...
            self.trigger_time = Lecroy23Be.TriggerTimestamp(self._io, self, self._root)
            self.acq_duration = self._io.read_f4be()
            self.record_type = KaitaiStream.resolve_enum(Lecroy23Be.RecordTypeEnum, self._io.read_u2be())
            self.processing_done = KaitaiStream.resolve_enum(Lecroy23Be.ProcessingDoneEnum, self._io.read_u2be())
            self.reserved5 = self._io.read_s2be()
            self.ris_sweeps = self._io.read_s2be()
            self.timebase = self._io.read_u2be()
            self.vert_coupling = KaitaiStream.resolve_enum(Lecroy23Be.VertCouplingEnum, self._io.read_u2be())
            self.probe_att = self._io.read_f4be()
            self.fixed_vert_gain = self._io.read_u2be()
            self.bandwidth_limit = KaitaiStream.resolve_enum(Lecroy23Be.BandwidthLimitEnum, self._io.read_u2be())
            self.vertical_vernier = self._io.read_f4be()
            self.acq_vert_offset = self._io.read_f4be()
            self.wave_source = KaitaiStream.resolve_enum(Lecroy23Be.WaveSourceEnum, self._io.read_u2be())
        ...
        def is_16bit(self):
            ...
            self._m_is_16bit = self.comm_type == Lecroy23Be.CommTypeEnum.word
    ...
    def trigtime_array(self):
        ...
        if self.wavedesc.trigtime_array_len > 0:
            ...
            for i in range(self.wavedesc.trigtime_array_len // 16):
                self._m_trigtime_array.append(Lecroy23Be.TrigtimeEntry(self._io, self, self._root))
