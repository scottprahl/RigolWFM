...
class SiglentV6(KaitaiStruct):
    ...
    def __init__(self, _io, _parent=None, _root=None):
        super(SiglentV6, self).__init__(_io)
        self._parent = _parent
        self._root = _root or self
        self._read()

    def _read(self):
        self.file_header = SiglentV6.FileHeader(self._io, self, self._root)
        self.waveforms = []
        for i in range(self.file_header.wave_number):
            self.waveforms.append(SiglentV6.Waveform(self._io, self, self._root))
    ...
    class FileHeader(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(SiglentV6.FileHeader, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class Waveform(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(SiglentV6.Waveform, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()

        def _read(self):
            self.header = SiglentV6.WaveformHeader(self._io, self, self._root)
    ...
    class WaveformHeader(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(SiglentV6.WaveformHeader, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
