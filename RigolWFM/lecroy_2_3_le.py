...
class Lecroy23Le(KaitaiStruct):
    ...
    def __init__(self, _io, _parent=None, _root=None):
        super(Lecroy23Le, self).__init__(_io)
        self._parent = _parent
        self._root = _root or self
        self._read()

    def _read(self):
        self.wavedesc = Lecroy23Le.Wavedesc(self._io, self, self._root)
    ...
    class TriggerTimestamp(KaitaiStruct):
        ...
        """
        def __init__(self, _io, _parent=None, _root=None):
            super(Lecroy23Le.TriggerTimestamp, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class TrigtimeEntry(KaitaiStruct):
        ...
        """
        def __init__(self, _io, _parent=None, _root=None):
            super(Lecroy23Le.TrigtimeEntry, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class Wavedesc(KaitaiStruct):
        ...
        """
        def __init__(self, _io, _parent=None, _root=None):
            super(Lecroy23Le.Wavedesc, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
        ...
        def _read(self):
            ...
            self.comm_type = KaitaiStream.resolve_enum(Lecroy23Le.CommTypeEnum, self._io.read_u2le())
            self.comm_order = KaitaiStream.resolve_enum(Lecroy23Le.CommOrderEnum, self._io.read_u2le())
            ...
            self.trigger_time = Lecroy23Le.TriggerTimestamp(self._io, self, self._root)
            self.acq_duration = self._io.read_f4le()
            self.record_type = KaitaiStream.resolve_enum(Lecroy23Le.RecordTypeEnum, self._io.read_u2le())
            self.processing_done = KaitaiStream.resolve_enum(Lecroy23Le.ProcessingDoneEnum, self._io.read_u2le())
            self.reserved5 = self._io.read_s2le()
            self.ris_sweeps = self._io.read_s2le()
            self.timebase = self._io.read_u2le()
            self.vert_coupling = KaitaiStream.resolve_enum(Lecroy23Le.VertCouplingEnum, self._io.read_u2le())
            self.probe_att = self._io.read_f4le()
            self.fixed_vert_gain = self._io.read_u2le()
            self.bandwidth_limit = KaitaiStream.resolve_enum(Lecroy23Le.BandwidthLimitEnum, self._io.read_u2le())
            self.vertical_vernier = self._io.read_f4le()
            self.acq_vert_offset = self._io.read_f4le()
            self.wave_source = KaitaiStream.resolve_enum(Lecroy23Le.WaveSourceEnum, self._io.read_u2le())
        ...
        def is_16bit(self):
            ...
            self._m_is_16bit = self.comm_type == Lecroy23Le.CommTypeEnum.word
    ...
    def trigtime_array(self):
        ...
        if self.wavedesc.trigtime_array_len > 0:
            ...
            for i in range(self.wavedesc.trigtime_array_len // 16):
                self._m_trigtime_array.append(Lecroy23Le.TrigtimeEntry(self._io, self, self._root))
