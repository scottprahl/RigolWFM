...
class Bin70008000(KaitaiStruct):
    ...
    def __init__(self, _io, _parent=None, _root=None):
        super(Bin70008000, self).__init__(_io)
        self._parent = _parent
        self._root = _root or self
        self._read()

    def _read(self):
        self.file_header = Bin70008000.FileHeader(self._io, self, self._root)
        self.waveforms = []
        for i in range(self.file_header.n_waveforms):
            self.waveforms.append(Bin70008000.Waveform(self._io, self, self._root))
    ...
    class DataHeader(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(Bin70008000.DataHeader, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
        ...
        def _read(self):
            ...
            self.buffer_type = KaitaiStream.resolve_enum(Bin70008000.BufferTypeEnum, self._io.read_u2le())
    ...
    class FileHeader(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(Bin70008000.FileHeader, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class Waveform(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(Bin70008000.Waveform, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()

        def _read(self):
            self.wfm_header = Bin70008000.WaveformHeader(self._io, self, self._root)
            self.data_header = Bin70008000.DataHeader(self._io, self, self._root)
    ...
    class WaveformHeader(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(Bin70008000.WaveformHeader, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
        ...
        def _read(self):
            self.header_size = self._io.read_u4le()
            self.waveform_type = KaitaiStream.resolve_enum(Bin70008000.WaveformTypeEnum, self._io.read_u4le())
            ...
            self.x_units = KaitaiStream.resolve_enum(Bin70008000.UnitEnum, self._io.read_u4le())
            self.y_units = KaitaiStream.resolve_enum(Bin70008000.UnitEnum, self._io.read_u4le())
