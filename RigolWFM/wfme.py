...
class Channel():
    ...
    def __str__(self):
        ...
        s += "        Delta  = " + engineering_string(self.time_delta) + "s\n"
        s += "    Raw volts:\n"
        s += "        Points = %d\n" % self.points
        if self.raw:
            s += "        [%d,%d,%d,...,%d,%d]\n" % (
...
class ChannelE(Channel):
    ...
    def __init__(self, w, ch=1):
        ...
        if ch == 1:
            ...
            self.volt_offset = w.header.ch1_volt_shift * w.header.ch1_volt_scale / 50
            ...
            self.volt_offset = w.header.ch2_volt_shift * w.header.ch2_volt_scale / 50
            self.time_delta = 1/w.header.sample_rate
