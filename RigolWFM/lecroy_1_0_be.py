...
class Lecroy10Be(KaitaiStruct):
    ...
    def __init__(self, _io, _parent=None, _root=None):
        super(Lecroy10Be, self).__init__(_io)
        self._parent = _parent
        self._root = _root or self
        self._read()

    def _read(self):
        self.wavedesc = Lecroy10Be.Wavedesc(self._io, self, self._root)
    ...
    class TriggerTimestamp(KaitaiStruct):
        ...
        def __init__(self, _io, _parent=None, _root=None):
            super(Lecroy10Be.TriggerTimestamp, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class TrigtimeEntry(KaitaiStruct):
        ...
        def __init__(self, _io, _parent=None, _root=None):
            super(Lecroy10Be.TrigtimeEntry, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class Wavedesc(KaitaiStruct):
        ...
        """
        def __init__(self, _io, _parent=None, _root=None):
            super(Lecroy10Be.Wavedesc, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
        ...
        def _read(self):
            ...
            self.comm_type = KaitaiStream.resolve_enum(Lecroy10Be.CommTypeEnum, self._io.read_u2be())
            self.comm_order = KaitaiStream.resolve_enum(Lecroy10Be.CommOrderEnum, self._io.read_u2be())
            ...
            self.trigger_time = Lecroy10Be.TriggerTimestamp(self._io, self, self._root)
            ...
            self.vert_coupling = KaitaiStream.resolve_enum(Lecroy10Be.VertCouplingEnum, self._io.read_u2be())
            self.probe_att = self._io.read_f4be()
            self.fixed_vert_gain = self._io.read_u2be()
            self.bandwidth_limit = KaitaiStream.resolve_enum(Lecroy10Be.BandwidthLimitEnum, self._io.read_u2be())
        ...
        def is_16bit(self):
            ...
            self._m_is_16bit = self.comm_type == Lecroy10Be.CommTypeEnum.word
    ...
    def trigtime_array(self):
        ...
        if self.wavedesc.trigtime_array_len > 0:
            ...
            for i in range(self.wavedesc.trigtime_array_len // 16):
                self._m_trigtime_array.append(Lecroy10Be.TrigtimeEntry(self._io, self, self._root))
